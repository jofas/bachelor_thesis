...
\begin{abstract}%
This thesis discusses how the abstention rule of an
abstaining classifier can be determined based on a reward
setting.
...
This thesis describes the underlying reward setting and
...
idea of using reward instead of cost comes---are shown.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE{sort $P$ based on the scores. In ascending order
           ...
           descending order}
    \STATE{$R := \text{scores from } P \times \rho(P_{i1},
           P_{i2}); i=1,\dots,n$}
